function [M_c, R_c, P_c, d_S, rho_mantle] = fn_get_C_m_implications(...
    ...
    M_c = nan(length(C_m), length(m_frac_sulfide));
    R_c = nan(length(C_m), length(m_frac_sulfide));
    P_c = nan(length(C_m), length(m_frac_sulfide));
    rho_mantle = nan(length(C_m), length(m_frac_sulfide));
    d_S = nan(length(C_m), length(m_frac_sulfide));
    for i=1:length(C_m)
            ...
            if dispersed
                ...
                for k=1:5
                    ...
                    core_params = fn_core_params(...
                        0, C_m(i), rho_mantle(i, j), rho_sulfide(j), rho_cr, ...
                        R_S, R_m);
                ...
                for k=1:5
                    core_params = fn_core_params(...
                        0, C_m(i), rho_silicate(j), rho_sulfide(j), rho_cr, ...
                        R_S, R_m);
                    if m_frac_sulfide(j)==0
                        break
                    end
